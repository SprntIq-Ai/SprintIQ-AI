...
export const LeaderboardPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const handleMutation = (e: any) => {
      if (e.detail?.entity === 'tasks' || e.detail?.entity === 'sprints') {
        loadData();
      }
    };
  ...
  return (
    <div className="space-y-8">
      ...
      <GlassCard className="space-y-4">
        ...
        {myBadges.length === 0 ? (
          <p className="text-xs text-slate-400 py-4">No achievement badges unlocked yet. Deliver sprint tasks to unlock badges.</p>
        ) : (
                ...
                <div key={b.id} className="p-4 rounded-xl bg-slate-50 border border-[rgba(var(--role-primary-rgb),0.30)] flex items-start gap-3">
                  ...
                  <div>
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-[10px] px-1.5 py-0.5 rounded bg-emerald-500/20 text-emerald-600 font-mono">UNLOCKED</span>
                    </div>
      ...
      <GlassCard className="space-y-4">
        <h3 className="text-lg font-bold text-slate-900 flex items-center gap-2">
          <Trophy className="w-5 h-5 text-amber-500" /> Organization Developer Leaderboard
        </h3>
              ...
              {leaderboard.length === 0 ? (
                <tr>
                  <td colSpan={8} className="py-8 text-center text-slate-400">
                    No developers currently ranked. Complete sprint tasks to appear on the leaderboard.
                  </td>
                </tr>
              ) : (
                  ...
                  <tr key={item.developer_id} className="hover:bg-slate-100/40">
                    <td className="py-3 px-4 font-bold text-slate-900">
                      {item.rank_position === 1 ? (
                        <span className="px-2 py-0.5 rounded bg-amber-500/20 text-amber-600 font-mono font-bold">🥇 #1</span>
                      ) : item.rank_position === 2 ? (
                        <span className="px-2 py-0.5 rounded bg-slate-200/50 text-slate-700 font-mono font-bold">🥈 #2</span>
                      ) : item.rank_position === 3 ? (
                        <span className="px-2 py-0.5 rounded bg-amber-700/20 text-amber-700 font-mono font-bold">🥉 #3</span>
                      ) : (
                    ...
                    <td className="py-3 px-4 text-center font-bold text-teal-600">{item.completed_tasks}</td>
                    <td className="py-3 px-4 text-center font-bold text-[var(--role-primary)]">{item.story_points} SP</td>
                    <td className="py-3 px-4 text-center font-bold text-emerald-600">{item.task_quality_score}%</td>
                    <td className="py-3 px-4 text-center font-bold" style={{ color: 'var(--role-primary)' }}>{item.on_time_delivery_rate}%</td>
                    <td className="py-3 px-4 text-center font-mono font-extrabold text-amber-600 text-sm">{item.overall_productivity_score}</td>
                    <td className="py-3 px-4 text-right">
                      <div className="flex justify-end gap-1 flex-wrap">
                        {item.badges && item.badges.map((bg, i) => (
